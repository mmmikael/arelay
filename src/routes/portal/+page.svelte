...
<div class="hidden h-full sm:flex sm:items-center sm:justify-center">
	{#if data.sessions.length === 0}
		<div
			class="max-w-lg rounded-xl border border-slate-100 bg-white p-8 shadow-[0_4px_20px_rgba(0,0,0,0.04)] dark:border-slate-800 dark:bg-slate-900 dark:shadow-none"
		>
			...
			<div
				class="mt-6 space-y-2 rounded-xl border border-slate-100 bg-slate-50 p-4 text-xs text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300"
			>
...
&#123; "encrypted": true, "ciphertext_base64": "...", ... &#125;</pre>
				<p class="text-slate-500 dark:text-slate-400">
					Use the agent-relay upload script. Plaintext delivery returns 400; missing E2EE setup
					returns 428.
				</p>
			</div>
		</div>
