<script lang="ts">
	...
	onMount(() => {
		...
		if ($page.url.searchParams.get('checkout') === 'success') {
			notice = 'Payment received — thank you! Your plan updates as soon as Stripe confirms it.';
			// The plan flips via webhook moments after redirect; refresh twice to catch it.
			invalidate('account:billing');
			setTimeout(() => invalidate('account:billing'), 3000);
		}
	});

	const currentPlan = $derived(
		data.billing && isPlanId(data.billing.plan) ? data.billing.plan : 'free'
	);
	let billingRedirectBusy = $state(false);

	async function openBillingPortal() {
		if (billingRedirectBusy) return;
		billingRedirectBusy = true;
		error = '';
		try {
		...
		{#if data.billing?.enabled}
			...
			>
				<div class="flex items-start gap-3">
					...
					<div class="min-w-0 flex-1">
						<div class="flex flex-wrap items-center justify-between gap-2">
							<h2 class="text-sm font-semibold text-slate-900 dark:text-slate-100">Plan</h2>
							<span
								class="rounded-full px-2.5 py-0.5 text-xs font-semibold {isPaidPlan(currentPlan)
									? 'bg-blue-100 text-blue-800 dark:bg-blue-500/15 dark:text-blue-200'
									: 'bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300'}"
							>
								{PLAN_LABELS[currentPlan]}{currentPlan === 'founding' ? ' · lifetime' : ''}
							</span>
						</div>
						{#if data.billing.subscriptionStatus === 'past_due'}
							<p
								class="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-2 text-xs text-amber-900 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-100"
							>
								Your last payment failed. Update your payment method in Manage billing to keep Pro.
							</p>
						{/if}
						<p class="mt-2 text-xs text-slate-500 dark:text-slate-400">
							{#if currentPlan === 'free'}
						...
						<div class="mt-3 flex flex-wrap gap-2">
							{#if currentPlan === 'free'}
								<Button href="/pricing" size="sm">See plans & upgrade</Button>
							{/if}
							{#if data.billing.hasStripeCustomer}
								<Button
									variant="outline"
									size="sm"
									disabled={billingRedirectBusy}
									onclick={openBillingPortal}
								>
									<ExternalLink class="mr-1.5 h-3.5 w-3.5" />
									{billingRedirectBusy ? 'Opening…' : 'Manage billing'}
								</Button>
							{/if}
						</div>
					</div>
